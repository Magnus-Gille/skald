...
A small e-paper display: Raspberry Pi 3A+ (host `skald`) + Waveshare 2.13" V4 panel (250×122, 1-bit). Skald is a **broadcast surface** that agents write to over an HTTP MCP. The Pi does not think; it renders.
...
- **Decisions live with the agent.** When an agent wants to show something, it composes the verse (or footer, or whatever) and calls `display_set_verse` / `display_set_footer`. Cadence is the agent's choice — ad-hoc, scheduled, prompted, doesn't matter.
...
A surface for the agent to be quiet on. Not a status dashboard. Not an autonomous voice. A small slow channel from the agent to Magnus, opened only when there's something worth saying.
