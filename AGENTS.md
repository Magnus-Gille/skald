...
A small e-paper display: Raspberry Pi 3A+ (host `skald`) + Waveshare 2.13" V4 panel (250×122, 1-bit). Skald is a **broadcast surface** that any agent — primarily Codex — writes to over an HTTP MCP. The Pi does not think; it renders.
...
- **Decisions live with the agent.** When Codex wants to show something, Codex composes the verse (or footer, or whatever) and calls `display_set_verse` / `display_set_footer`. Cadence is the agent's choice — ad-hoc, scheduled, prompted, doesn't matter.
...
A surface for the agent to be quiet on. Not a status dashboard. Not an autonomous voice. A small slow channel from Codex to Magnus, opened only when there's something worth saying.
